...
const RADIANCE_AFFECTED: readonly string[] = [
  // Faithful port of WynnBuilder's `radiance_affected` (builder_graph.js).
  // Excludes the commented-out entries there (hp, skillpoints, xpb/lb, the
  // *AddMin/Max raw-damage pairs, etc.) and the two non-numeric structural keys
  // (`fixID`, `category`) which are not stats in our model.
  'fDef',
  'wDef',
  'aDef',
  'tDef',
  ...
  'sprintReg',
  'jh',
  // wynn2 per-element melee/spell damage
  'eMdPct',
  'eMdRaw',
  'eSdPct',
  'eSdRaw',
