...
describe('resolveImport', () => {
  it('happy path: helmet → slot 0, overrides, no powders', () => {
    ...
    const r = resolveImport(blocks('Morph-Stardust', 0, [{ kind: 12, roll: 50 }, { kind: 34, roll: 100 }]), ctx, idKeys, new Set(), mockGetRange)
  ...
  it('negative-base clamp: percentile → raw stays in [min(min,max), max(min,max)]', () => {
    ...
    const r = resolveImport(blocks('BadRing', 0, [{ kind: 50, roll: 100 }]), ctx, localKeys, new Set(), mockGetRange)
    expect(r.ok).toBe(true)
    if (r.ok) {
  ...
  it('unknown item name → ImportError', () => {
    ...
    const r = resolveImport(blocks('NotARealItem'), ctx, idKeys, new Set(), mockGetRange)
  ...
  it('unknown stat byte → skipped with warning, others applied', () => {
    ...
    const r = resolveImport(blocks('H', 0, [{ kind: 12, roll: 100 }, { kind: 200, roll: 50 }]), ctx, idKeys, new Set(), mockGetRange)
    expect(r.ok).toBe(true)
    if (r.ok) {
      expect(r.row.overrides.get('str')).toBe(10)
  ...
  it('v3 name has no shorthand mapping → skipped with warning', () => {
    const ctx = mkCtx([mkItem(7, 'H', 'helmet', { str: { min: 1, max: 10, raw: 10 } })])
    const r = resolveImport(blocks('H', 0, [{ kind: 99, roll: 50 }, { kind: 12, roll: 100 }]), ctx, idKeys, new Set(), mockGetRange)
    expect(r.ok).toBe(true)
    if (r.ok) {
      expect(r.row.overrides.get('str')).toBe(10)
  ...
  it('non-GEAR type byte → ImportError', () => {
    ...
    const r = resolveImport(blocks('H', 1), ctx, idKeys, new Set(), mockGetRange)
    expect(r.ok).toBe(false)
    if (!r.ok)
  ...
  it('rings: first → slot 4, second → slot 5, third → warning + slot 4', () => {
    ...
    const a = resolveImport(blocks('R'), ctx, idKeys, occupied, mockGetRange)
    if (a.ok)
      occupied.add(a.row.slot)
    const b = resolveImport(blocks('R'), ctx, idKeys, occupied, mockGetRange)
    if (b.ok)
      occupied.add(b.row.slot)
    const c = resolveImport(blocks('R'), ctx, idKeys, occupied, mockGetRange)
  ...
  it('powders: element/tier pairs → numeric ids in slot powder format', () => {
    ...
    const r = resolveImport(blocks('W', 0, [], [{ element: 0, tier: 1 }, { element: 2, tier: 3 }]), ctx, idKeys, new Set(), mockGetRange)
    expect(r.ok).toBe(true)
    if (r.ok)
...
describe('parseAndResolveAll', () => {
  it('routes two ring lines to slots 4 then 5', () => {
    ...
    const result = parseAndResolveAll('Moon Pool Circlet\nMoon Pool Circlet', ctx, idKeys, fakeParse, mockGetRange)
  ...
  it('reports per-line errors without aborting', () => {
    ...
    const fakeParse = (line: string) => blocks(line)
    const result = parseAndResolveAll('Helm\nNotReal', ctx, idKeys, fakeParse, mockGetRange)
