...
function toRows(record: Record<string, { min: number, max: number, raw: number }>): IdRow[] {
  return Object.entries(record).map(([key, r]) => {
...
const allRows = computed<IdRow[]>(() => [
  ...toRows(props.charm.base),
  ...toRows(props.charm.identifications),
])
    ...
    <div class="tt-inner" :style="innerStyle">
      <div class="tt-header">
        ...
        <div class="tt-headtext">
          <span class="tt-name" :style="{ color: theme.color }">{{ charm.displayName }}</span>
          <div class="tt-tags">
            <span class="tt-tag" :style="{ background: theme.color }">{{ normalizedTier.toLowerCase() }}</span>
            <span class="tt-tag" :style="{ background: theme.light }">charm</span>
          </div>
        </div>
      </div>
      ...
      <div class="tt-meta-row">
        ...
        <span class="tt-muted">{{ charm.level }}</span>
      </div>

      <template v-if="allRows.length">
        <div class="tt-sep" :style="sepStyle()" />
        <ul class="tt-ids">
          <li v-for="row in allRows" :key="row.label" class="tt-id">
            <span class="tt-id-name">{{ row.label }}</span>
            <span class="tt-id-range">
              <span :style="{ color: row.color }">{{ fmt(row.min, row.unit) }}</span>
              <span class="tt-id-arrow">to</span>
              <span :style="{ color: row.color }">{{ fmt(row.max, row.unit) }}</span>
            </span>
          </li>
        </ul>
      </template>
