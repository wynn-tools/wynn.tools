...
import type { ApiBuildCredit, ApiItem } from '~/composables/useApi'
import { computed } from 'vue'
import Byline from '~/components/BuildDetails/Byline.vue'
import CreditsPicker from '~/components/BuildDetails/CreditsPicker.vue'
import NotesEditor from '~/components/BuildDetails/NotesEditor.vue'
import NotesView from '~/components/BuildDetails/NotesView.vue'
import TagChips from '~/components/BuildDetails/TagChips.vue'
import TagPicker from '~/components/BuildDetails/TagPicker.vue'
...
defineEmits<{
  ...
  updateCredits: [credits: ApiBuildCredit[]]
  updateNotes: [notes: string | null]
}>()

const hasTags = computed(() => !!props.item.tags && props.item.tags.length > 0)
const hasNotes = computed(() => !!props.item.notes && props.item.notes.trim().length > 0)

const visibilityLabel = computed(() => {
  const v = props.item.visibility
    ...
    <div class="header-inner">
      ...
      <div class="header-row header-row-2">
        ...
        <div class="header-row-2-right">
          ...
          />
          <TagChips v-else-if="hasTags" :tags="item.tags" :max="6" />
        </div>
      </div>

      <div v-if="isOwner" class="header-row header-row-3">
        <NotesEditor :notes="item.notes" @save="(n: string | null) => $emit('updateNotes', n)" />
      </div>
      <div v-else-if="hasNotes" class="header-row header-row-3">
        <NotesView :markdown="item.notes" />
      </div>
    </div>
  </header>
</template>
