...
<template>
  <div class="list-page">
    ...
    <div v-else class="card-list">
      <div v-for="i in items" :key="i.id" class="manage-card">
        <NuxtLink :to="`/c/${i.id}`" class="card-link">
          <input
            v-if="editingId === i.id"
            v-model="editingName"
            class="rename-input"
            ...
            @click.prevent.stop
          >
          <span v-else class="card-name" @click.prevent.stop="startRename(i)">{{ i.name }}</span>
        </NuxtLink>

        <div class="card-actions">
          <span class="card-version">{{ i.gameVersion }}</span>
          <select
