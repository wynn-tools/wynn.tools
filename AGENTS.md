...
Nuxt 4 + Vue 3 + Pinia + Tailwind CSS v4 + Reka UI. Tests with Vitest (node environment). SSR is on by default; pages opt out per-route (`defineRouteRules({ ssr: false })`) only when they can't render server-side (e.g. Leaflet/Pixi map). OG images generated server-side via `nuxt-og-image` with the **Takumi** renderer (satori + resvg are uninstalled; OG components use the `*.takumi.vue` suffix). The backend (`api/`) is a separate Hono + Drizzle + Postgres service — see "Accounts, sharing & prices" below.
